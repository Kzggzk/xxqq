...
- Latest local verified version / 最近本地验证版本: `1.71` public-copy cleanup and internal infrastructure docs
- Latest production verified version / 最近生产验证版本: `1.71`
- iOS companion / iOS 伴生版本: `0.7` native SwiftUI scaffold at `/Users/fangbao/kzg-options-minute-site/ios/KZGOptionHouse`
- Latest production correction / 最近生产修正: `v1.71 public-copy scrub production deploy`
- Latest local correction / 最近本地修正: `v1.71 public homepage internal-copy scrub`
- Latest unique deploy verified / 最近验证唯一部署: <https://6a16bc992cdd920079852497--kzg-option-house.netlify.app/>
...
## v1.71 - 2026-05-27 17:45 Asia/Shanghai - public-copy scrub and infrastructure docs

中文:

- 改动: 公开 Web UI 从 `1.70` 提到 `1.71`。`public/app.js`、`public/index.html`、`public/styles.css` 清理公开页面里不该给用户看的对内话术：移除“产品阅读路径 / 三段式 / 不绕路 / 预留 / 后端确认 / 未来实时 / 专门页面 / 产品轮廓”等表达；中段改成普通的“期权流样张 / Flow sample”数据展示；Realtime tape 不再带 `is-realtime-gated` blur class，也移除遮罩文案。`docs/COMMERCIAL_INFRASTRUCTURE_PLAN.md` 新增 Network Solutions 域名、用户登录、Supabase schema、entitlement 和 hard-stop 路线，只在内部 docs 里记录。
- 原因: Fangbao 明确指出公开页面出现了内部产品风格描述和规划话术，这些内容只应该存在于 documentation/owner handoff，不应该进产品页面。
- 边界: 自动心跳 `kzg-option-house-pro-evolution-loop` 已按 Fangbao 要求删除，不再每 15 分钟自动推进。没有购买域名、没有改 DNS、没有创建 Supabase 表、没有使用 API key、没有配置支付、没有提交 raw generated data。
- 验证: `node --check public/app.js` 通过；`git diff --check -- public/app.js public/index.html public/styles.css docs` 通过；公开 UI 源文件扫描不再命中内部商业/provider/API/payment/domain 规划词。构建生成 `505` 天 payload，latest `2026-05-22`，pack asset `kzg-frame-cb9ed4614a48.js`；`per_day_to_dist` 返回 `copied=505`。Browser 本地 `http://127.0.0.1:4211/` 验证 `1.71 · 505/505 complete`、内部话术 false、overflow 0、`.is-realtime-gated` 0、`.is-blurred` 0、`.is-locked` 0、`.terminal-veil` 0、console issue 0。Playwright 本地桌面/手机同样通过，PNG `/tmp/kzg-option-house-v171-export.png` 为 `1,482,138` bytes。生产 Browser 与 Playwright 验证同样通过：生产 `sourcePath` 为 `1.71 · 505/505 complete`，公开话术 false，桌面/手机 overflow 0，gated/blurred/locked/veil 0，console issue 0，PNG `/tmp/kzg-option-house-v171-prod-export.png` 为 `1,482,138` bytes。生产 `/data/index.json` 与 `/assets/kzg-pack.js` 仍为 `404`。Apple Notes 置顶 `CHANGLOG 期权终端` 已同步，`updated=1`，`created=0`，正文 `223303` chars。
- 部署: 已部署生产。Production URL <https://kzg-option-house.netlify.app/>；unique deploy <https://6a16bc992cdd920079852497--kzg-option-house.netlify.app/>；deploy id `6a16bc992cdd920079852497`。
- 下一步: 不恢复自动心跳。Network Solutions、登录、Supabase、支付和真实 API 只在 docs 里继续审阅；进入真实操作前必须再次问 Fangbao。

English:

- Change: Public Web UI moves from `1.70` to `1.71`. `public/app.js`, `public/index.html`, and `public/styles.css` scrub internal copy from the public surface: phrases like product reading path, three-sector/no-detour, reserve, backend approval, future realtime, dedicated page, and product outline were removed. The middle section is now plain `Options flow sample` data display. The realtime tape no longer uses the `is-realtime-gated` blur class, and the overlay copy was removed. `docs/COMMERCIAL_INFRASTRUCTURE_PLAN.md` adds internal-only Network Solutions domain, login/auth, Supabase schema, entitlement, and hard-stop planning.
- Reason: Fangbao explicitly called out internal product-style and planning wording leaking onto the public page. Those belong only in documentation and owner handoff, not the product UI.
- Boundary: The automation heartbeat `kzg-option-house-pro-evolution-loop` was deleted by Fangbao request and no longer runs every 15 minutes. No domain was purchased, no DNS changed, no Supabase table was created, no API key was used, no payment route was configured, and no raw generated data was committed.
- Verification: `node --check public/app.js` passed; `git diff --check -- public/app.js public/index.html public/styles.css docs` passed; public UI source scan no longer matches internal commercial/provider/API/payment/domain planning terms. Build produced `505` days, latest `2026-05-22`, pack asset `kzg-frame-cb9ed4614a48.js`; `per_day_to_dist` returned `copied=505`. Browser local `http://127.0.0.1:4211/` verified `1.71 · 505/505 complete`, internal-copy scan false, overflow 0, `.is-realtime-gated` 0, `.is-blurred` 0, `.is-locked` 0, `.terminal-veil` 0, console issue 0. Playwright local desktop/phone matched and PNG `/tmp/kzg-option-house-v171-export.png` was `1,482,138` bytes. Production Browser and Playwright also passed: production `sourcePath` is `1.71 · 505/505 complete`, public-copy scan false, desktop/phone overflow 0, gated/blurred/locked/veil 0, console issue 0, PNG `/tmp/kzg-option-house-v171-prod-export.png` was `1,482,138` bytes. Production `/data/index.json` and `/assets/kzg-pack.js` remain `404`. Apple Notes pinned `CHANGLOG 期权终端` synced with `updated=1`, `created=0`, `223303` body characters.
- Deploy: Deployed to production. Production URL <https://kzg-option-house.netlify.app/>; unique deploy <https://6a16bc992cdd920079852497--kzg-option-house.netlify.app/>; deploy id `6a16bc992cdd920079852497`.
- Next: Do not restore the heartbeat automation. Network Solutions, auth, Supabase, payments, and real API integration stay in docs until Fangbao explicitly approves real action.

## v1.70 - 2026-05-27 17:12 Asia/Shanghai - momentum queue compass

中文:

- 改动: 公开 Web UI 从 `1.69` 提到 `1.70`。`public/app.js` 在 `renderSymbolMomentum()` 的动量 summary 和队列之间新增 `momentumQueueCompass()` 与 `momentumCompassStep()`，把资金锚点、CP 极值、当前行压成一条“动量队列读法”。`public/styles.css` 新增 `.momentum-queue-compass`、`.momentum-compass-shell`、`.momentum-compass-steps`、`.momentum-compass-jump` 的桌面、手机、深色样式。
- 体验: v1.69 已经让 RGTI 单标的镜头回到底部动量队列；v1.70 再把队列读法压短：先看权利金锚，再看 CP 极值，最后回到当前 active 行。手机端从“队列堆叠”变成“资金先读 / CP 极值 / 当前行”的三步阅读，不新增任何 lock、provider、订阅、付费、API 或账号路线。
- 改动文件: `public/app.js`、`public/styles.css`、`docs/CHANGELOG.md`、`docs/CHANGLOG_OPTION_TERMINAL.md`、`docs/DENSE_VERSIONING.md`、`docs/HANDOFF_FOR_OTHER_CODEX.md`、`docs/MASSIVE_REALTIME_PRODUCT_PLAN.md`、`docs/PLUGIN_SERVICE_STATUS.md`、`docs/IOS_COMPANION_PLAN.md`、`docs/CHANGLOG_OPTION_TERMINAL_MINDMAP.opml`。
- 验证: `node --check public/app.js` 通过；`git diff --check -- public/app.js public/styles.css` 通过；公开风险词扫描 `public/app.js public/index.html public/styles.css` 无命中。`python3 scripts/build_payload.py` 生成 `505` 天 payload，latest `2026-05-22`，analytics days `505`，analytics symbols `98`，pack asset `kzg-frame-95df00c31270.js`；`python3 scripts/per_day_to_dist.py` 返回 `copied=505`。Browser 插件本轮可用，打开 `http://127.0.0.1:4210/` 验证标题、`1.70 · 505/505 complete`、momentum compass 1、steps 3、jump 1；点击轮动接力后 focus `RGTI`，RGTI queue row 1，点击队列后 active RGTI row 1，点击 compass jump 后 rotation focus 1，横向溢出 0，旧 `.is-blurred`/`.is-locked` 0，future gate 1，公开 provider/API/payment 字符 false，console issue 0。Playwright 对 dist `http://127.0.0.1:4210/` 验证桌面 `1440x1100` 和手机 `390x844`，结果一致；PNG 导出成功，`/tmp/kzg-option-house-v170-export.png`，大小 `1,482,138` bytes。截图证据: `/tmp/kzg-option-house-v170-dist-desktop.png`、`/tmp/kzg-option-house-v170-compass-desktop.png`、`/tmp/kzg-option-house-v170-dist-mobile.png`、`/tmp/kzg-option-house-v170-compass-mobile.png`、`/tmp/kzg-option-house-v170-export.png`。
- 部署: 本轮不部署生产。生产仍是 v1.68，生产站 <https://kzg-option-house.netlify.app/>，唯一部署 <https://6a16acc1dbce120ee54cac50--kzg-option-house.netlify.app/>。不部署原因：v1.69/v1.70 是 v1.68 生产部署后的本地开放历史连续读盘增强，未发现公开风险修复需求。
- 下一步: v1.71 继续 Web，优先继续压缩开放历史层手机高度，把动量队列、轮动象限和资金锚点之间的交互节奏再做得更像“读盘路径”而不是模块堆叠。iOS companion 保持 `0.7`，下一次默认同步约在 Web v1.72。

English:

- Change: Public Web UI moves from `1.69` to `1.70`. `public/app.js` adds `momentumQueueCompass()` and `momentumCompassStep()` between the momentum summary and queue inside `renderSymbolMomentum()`, compressing capital anchor, CP extreme, and active row into a `Momentum queue read` bridge. `public/styles.css` adds desktop, phone, and dark-mode styling for `.momentum-queue-compass`, `.momentum-compass-shell`, `.momentum-compass-steps`, and `.momentum-compass-jump`.
- Experience: v1.69 returned the RGTI symbol lens into the lower momentum queue; v1.70 makes that queue readable in three steps: capital anchor first, CP extreme second, active row last. On phone, the queue now reads as `Capital first / CP edge / Active row`, not a dense stack. It adds no lock, provider route, subscription, payment, API, or account wording.
- Changed files: `public/app.js`, `public/styles.css`, `docs/CHANGELOG.md`, `docs/CHANGLOG_OPTION_TERMINAL.md`, `docs/DENSE_VERSIONING.md`, `docs/HANDOFF_FOR_OTHER_CODEX.md`, `docs/MASSIVE_REALTIME_PRODUCT_PLAN.md`, `docs/PLUGIN_SERVICE_STATUS.md`, `docs/IOS_COMPANION_PLAN.md`, and `docs/CHANGLOG_OPTION_TERMINAL_MINDMAP.opml`.
- Verification: `node --check public/app.js` passed; `git diff --check -- public/app.js public/styles.css` passed; public-risk scan across `public/app.js public/index.html public/styles.css` had no matches. `python3 scripts/build_payload.py` produced a `505`-day payload, latest `2026-05-22`, analytics days `505`, analytics symbols `98`, pack asset `kzg-frame-95df00c31270.js`; `python3 scripts/per_day_to_dist.py` returned `copied=505`. Browser plugin was available this round and opened `http://127.0.0.1:4210/`, verifying title, `1.70 · 505/505 complete`, momentum compass 1, steps 3, jump 1; after rotation handoff focus was `RGTI`, RGTI queue row 1, active RGTI row 1 after queue jump, rotation focus 1 after compass jump, horizontal overflow 0, old `.is-blurred`/`.is-locked` 0, future gate 1, public provider/API/payment text false, and console issues 0. Playwright against dist `http://127.0.0.1:4210/` verified desktop `1440x1100` and phone `390x844` with the same result. PNG export succeeded at `/tmp/kzg-option-house-v170-export.png`, size `1,482,138` bytes. Screenshot evidence: `/tmp/kzg-option-house-v170-dist-desktop.png`, `/tmp/kzg-option-house-v170-compass-desktop.png`, `/tmp/kzg-option-house-v170-dist-mobile.png`, `/tmp/kzg-option-house-v170-compass-mobile.png`, and `/tmp/kzg-option-house-v170-export.png`.
- Deploy: Not deployed this round. Production remains v1.68 at <https://kzg-option-house.netlify.app/>, unique deploy <https://6a16acc1dbce120ee54cac50--kzg-option-house.netlify.app/>. Reason: v1.69/v1.70 are local open-history reading-continuity improvements after the v1.68 production deploy, with no public-risk fix required.
- Next: v1.71 should stay on Web and further compress phone height in the open historical layer, making the interaction between momentum queue, rotation quadrant, and capital anchor feel like a reading path instead of stacked modules. iOS companion remains `0.7`; the next default iOS sync is around Web v1.72.

## v1.69 - 2026-05-27 17:05 Asia/Shanghai - symbol-to-momentum queue handoff

中文:

- 改动: 公开 Web UI 从 `1.68` 提到 `1.69`。`public/app.js` 在单标的聚焦镜头底部新增 `symbolMomentumHandoff()` 和 `symbolMomentumStep()`，把当前标的、资金锚点、队列位置压成一条“标的到动量队列”接力。`renderSymbolMomentum()` 现在会把当前聚焦标的保留在底部动量队列里，即使它原本不在前 18 个默认行中；点击 `data-symbol` 时会同时重渲染单标的镜头和动量队列。`public/styles.css` 新增桌面、手机、深色样式。
- 体验: v1.68 解决“轮动扩散 -> RGTI 单标的镜头”；v1.69 继续解决“看完 RGTI 后下一步去哪”。用户现在从 RGTI 镜头可以回到底下开放的动量队列，看到 RGTI 行、全市场资金锚点和队列位置。它仍是开放历史层导读，不是实时权限、订阅、解锁、provider、账号或支付路线。
- 改动文件: `public/app.js`、`public/styles.css`、`docs/CHANGELOG.md`、`docs/CHANGLOG_OPTION_TERMINAL.md`、`docs/DENSE_VERSIONING.md`、`docs/HANDOFF_FOR_OTHER_CODEX.md`、`docs/MASSIVE_REALTIME_PRODUCT_PLAN.md`、`docs/PLUGIN_SERVICE_STATUS.md`、`docs/IOS_COMPANION_PLAN.md`、`docs/CHANGLOG_OPTION_TERMINAL_MINDMAP.opml`。
- 验证: `node --check public/app.js` 通过；`git diff --check -- public/app.js public/styles.css` 通过；公开风险词扫描 `public/app.js public/index.html public/styles.css` 无命中。`python3 scripts/build_payload.py` 生成 `505` 天 payload，latest `2026-05-22`，analytics days `505`，analytics symbols `98`，pack asset `kzg-frame-65a81a1ab0bf.js`；`python3 scripts/per_day_to_dist.py` 返回 `copied=505`。Browser 本地控制工具本轮未暴露，按前端验证流程退回 Playwright。Playwright 对 dist `http://127.0.0.1:4209/` 验证桌面 `1440x1100` 和手机 `390x844`：`sourcePath` 为 `1.69 · 505/505 complete`，symbol momentum handoff 1，steps 3，jump 1，rotation click 后 focus `RGTI`，RGTI queue row 1，点击回队列后 `.momentum-panel.history-scroll-focus = 1`、`.momentum-row.active[data-symbol="RGTI"] = 1`，横向溢出 0，旧 `.is-blurred`/`.is-locked` 0，future gate 1，公开 provider/API/payment 字符 false，console issue 0。PNG 导出成功，`/tmp/kzg-option-house-v169-export.png`，大小 `1,482,138` bytes。截图证据: `/tmp/kzg-option-house-v169-dist-desktop.png`、`/tmp/kzg-option-house-v169-handoff-desktop.png`、`/tmp/kzg-option-house-v169-dist-mobile.png`、`/tmp/kzg-option-house-v169-handoff-mobile.png`、`/tmp/kzg-option-house-v169-export.png`。
- 部署: 本轮不部署生产。生产仍是 v1.68，生产站 <https://kzg-option-house.netlify.app/>，唯一部署 <https://6a16acc1dbce120ee54cac50--kzg-option-house.netlify.app/>。不部署原因：v1.68 刚完成生产部署，v1.69 是本地开放历史层连续读盘增强，未发现公开风险修复需求。
- 下一步: v1.70 继续 Web，优先把动量队列与资金锚点/轮动象限之间的故事压得更短、更适合手机下拉阅读；iOS companion 保持 `0.7`，下一次默认同步约在 Web v1.72。

English:

- Change: Public Web UI moves from `1.68` to `1.69`. `public/app.js` adds `symbolMomentumHandoff()` and `symbolMomentumStep()` under the single-symbol lens, compressing current symbol, capital anchor, and queue position into a `Symbol to momentum queue` handoff. `renderSymbolMomentum()` now keeps the current focus symbol visible in the lower momentum queue even when it was not part of the default first 18 rows; `data-symbol` clicks now rerender both the symbol lens and momentum queue. `public/styles.css` adds desktop, phone, and dark-mode styling.
- Experience: v1.68 solved `rotation breadth -> RGTI symbol lens`; v1.69 solves what happens after reading RGTI. Users can now return from the RGTI lens into the open momentum queue and see the RGTI row, market-wide capital anchor, and queue position. This remains open historical guidance, not realtime entitlement, subscription, unlock, provider, account, or payment routing.
- Changed files: `public/app.js`, `public/styles.css`, `docs/CHANGELOG.md`, `docs/CHANGLOG_OPTION_TERMINAL.md`, `docs/DENSE_VERSIONING.md`, `docs/HANDOFF_FOR_OTHER_CODEX.md`, `docs/MASSIVE_REALTIME_PRODUCT_PLAN.md`, `docs/PLUGIN_SERVICE_STATUS.md`, `docs/IOS_COMPANION_PLAN.md`, and `docs/CHANGLOG_OPTION_TERMINAL_MINDMAP.opml`.
- Verification: `node --check public/app.js` passed; `git diff --check -- public/app.js public/styles.css` passed; public-risk scan across `public/app.js public/index.html public/styles.css` had no matches. `python3 scripts/build_payload.py` produced a `505`-day payload, latest `2026-05-22`, analytics days `505`, analytics symbols `98`, pack asset `kzg-frame-65a81a1ab0bf.js`; `python3 scripts/per_day_to_dist.py` returned `copied=505`. The Browser local-control tool was not exposed this round, so verification fell back to Playwright. Playwright against dist `http://127.0.0.1:4209/` verified desktop `1440x1100` and phone `390x844`: `sourcePath` showed `1.69 · 505/505 complete`, symbol momentum handoff 1, steps 3, jump 1, focus `RGTI` after rotation click, RGTI queue row 1, `.momentum-panel.history-scroll-focus = 1` after queue jump, `.momentum-row.active[data-symbol="RGTI"] = 1`, horizontal overflow 0, old `.is-blurred`/`.is-locked` 0, future gate 1, public provider/API/payment text false, and console issues 0. PNG export succeeded at `/tmp/kzg-option-house-v169-export.png`, size `1,482,138` bytes. Screenshot evidence: `/tmp/kzg-option-house-v169-dist-desktop.png`, `/tmp/kzg-option-house-v169-handoff-desktop.png`, `/tmp/kzg-option-house-v169-dist-mobile.png`, `/tmp/kzg-option-house-v169-handoff-mobile.png`, and `/tmp/kzg-option-house-v169-export.png`.
- Deploy: Not deployed this round. Production remains v1.68 at <https://kzg-option-house.netlify.app/>, unique deploy <https://6a16acc1dbce120ee54cac50--kzg-option-house.netlify.app/>. Reason: v1.68 was just deployed, and v1.69 is a local open-history reading-continuity improvement with no public-risk fix required.
- Next: v1.70 should stay on Web and further compress the story between momentum queue, capital anchors, and rotation quadrant for phone scrolling. iOS companion remains `0.7`; the next default iOS sync is around Web v1.72.

## v1.68 - 2026-05-27 16:34 Asia/Shanghai - rotation-to-symbol focus handoff and production deploy

中文:

- 改动: 公开 Web UI 从 `1.66` 提到 `1.68`；`1.67` 是 iOS 伴生同步点，所以 Web 版本号跳到本轮目标。`public/app.js` 在 `renderSymbolRotation()` 的轮动 lanes 后加入 `rotationFocusHandoff()` 和 `rotationFocusStep()`，把轮动扩散的领头标的、资金锚点、CP 两端压成一条“轮动到标的聚焦”桥。桥接按钮同时带 `data-symbol` 和 `data-scroll-sector="symbolFocus"`，点击后先把焦点标的设为 `RGTI`，再复用 v1.65 的面板聚焦滚动落到开放的单标的 90 日镜头。`public/styles.css` 新增桌面、手机、深色样式，并在手机端把 rotation map 和 lane rows 再压短。
- 体验: v1.66 已把日内压力桶接到轮动象限；v1.68 继续把轮动象限接到单标的聚焦。用户现在能按“压力桶 -> 轮动扩散 -> RGTI 单标的节奏”连续读完开放历史下半区，不需要在一堆模块里猜下一步看什么。它仍是公开历史层的导读，不是实时权限、订阅、解锁、provider 或支付路线。
- 改动文件: `public/app.js`、`public/styles.css`、`docs/CHANGELOG.md`、`docs/CHANGLOG_OPTION_TERMINAL.md`、`docs/DENSE_VERSIONING.md`、`docs/HANDOFF_FOR_OTHER_CODEX.md`、`docs/MASSIVE_REALTIME_PRODUCT_PLAN.md`、`docs/PLUGIN_SERVICE_STATUS.md`、`docs/IOS_COMPANION_PLAN.md`、`docs/CHANGLOG_OPTION_TERMINAL_MINDMAP.opml`。
- 验证: `node --check public/app.js` 通过；`git diff --check -- public/app.js public/styles.css` 通过。`python3 scripts/build_payload.py` 生成 `505` 天 payload，latest `2026-05-22`，analytics days `505`，analytics symbols `98`，pack asset `kzg-frame-78c457d24fa4.js`；`python3 scripts/per_day_to_dist.py` 返回 `copied=505`。Browser 本地控制工具本轮未暴露，按前端验证流程退回 Playwright。Playwright 对 dist `http://127.0.0.1:4208/` 验证桌面 `1440x1100` 和手机 `390x844`：`sourcePath` 为 `1.68 · 505/505 complete`，rotation focus handoff 1，steps 3，jump 1，jump symbol `RGTI`，点击后 `.momentum-panel.history-scroll-focus = 1` 且 `#symbolFocus` 为 `RGTI`，横向溢出 0，旧 `.is-blurred`/`.is-locked` 0，future gate 1，公开 provider/API/payment 字符 false，console issue 0。PNG 导出成功，`/tmp/kzg-option-house-v168-export.png`，大小 `1,482,138` bytes。截图证据: `/tmp/kzg-option-house-v168-dist-desktop.png`、`/tmp/kzg-option-house-v168-handoff-desktop.png`、`/tmp/kzg-option-house-v168-dist-mobile.png`、`/tmp/kzg-option-house-v168-handoff-mobile.png`、`/tmp/kzg-option-house-v168-export.png`。
- 部署: 已部署生产。生产站 <https://kzg-option-house.netlify.app/>；唯一部署 <https://6a16acc1dbce120ee54cac50--kzg-option-house.netlify.app/>；deploy id `6a16acc1dbce120ee54cac50`。线上 `app.js` 验证 `UI_VERSION = "1.68"`；线上 `/data/index.json` 和 `/assets/kzg-pack.js` 都是 `404`；线上 Playwright 桌面/手机验证与本地一致，PNG 导出 `1,482,138` bytes。
- 下一步: v1.69 继续 Web 层，优先把单标的聚焦与 momentum list 再接回资金锚点/轮动列表，减少底部“看完 RGTI 后下一步去哪”的断点。iOS companion 保持 `0.7`，下一次默认同步约在 Web v1.72。

English:

- Change: Public Web UI moves from `1.66` to `1.68`; `1.67` was the iOS companion sync checkpoint, so Web jumps to this target. `public/app.js` adds `rotationFocusHandoff()` and `rotationFocusStep()` after the rotation lanes in `renderSymbolRotation()`, compressing the rotation leader, capital anchor, and CP edges into a `Rotation to symbol lens` bridge. The jump button carries both `data-symbol` and `data-scroll-sector="symbolFocus"`, so tapping it sets focus to `RGTI` and reuses v1.65 panel-focus scrolling into the open 90-session symbol lens. `public/styles.css` adds desktop, phone, and dark-mode styling, and further compresses rotation map/lane rows on phones.
- Experience: v1.66 connected intraday pressure buckets into the rotation quadrant; v1.68 connects the rotation quadrant into single-symbol focus. Users can now read the lower open-history layer as `pressure bucket -> rotation breadth -> RGTI symbol rhythm` instead of guessing which dense module to inspect next. This remains public historical guidance, not realtime entitlement, subscription, unlock, provider, or payment routing.
- Changed files: `public/app.js`, `public/styles.css`, `docs/CHANGELOG.md`, `docs/CHANGLOG_OPTION_TERMINAL.md`, `docs/DENSE_VERSIONING.md`, `docs/HANDOFF_FOR_OTHER_CODEX.md`, `docs/MASSIVE_REALTIME_PRODUCT_PLAN.md`, `docs/PLUGIN_SERVICE_STATUS.md`, `docs/IOS_COMPANION_PLAN.md`, and `docs/CHANGLOG_OPTION_TERMINAL_MINDMAP.opml`.
- Verification: `node --check public/app.js` passed; `git diff --check -- public/app.js public/styles.css` passed. `python3 scripts/build_payload.py` produced a `505`-day payload, latest `2026-05-22`, analytics days `505`, analytics symbols `98`, pack asset `kzg-frame-78c457d24fa4.js`; `python3 scripts/per_day_to_dist.py` returned `copied=505`. The Browser local-control tool was not exposed this round, so verification fell back to Playwright. Playwright against dist `http://127.0.0.1:4208/` verified desktop `1440x1100` and phone `390x844`: `sourcePath` showed `1.68 · 505/505 complete`, rotation focus handoff 1, steps 3, jump 1, jump symbol `RGTI`, `.momentum-panel.history-scroll-focus = 1` after click with `#symbolFocus` set to `RGTI`, horizontal overflow 0, old `.is-blurred`/`.is-locked` 0, future gate 1, public provider/API/payment text false, and console issues 0. PNG export succeeded at `/tmp/kzg-option-house-v168-export.png`, size `1,482,138` bytes. Screenshot evidence: `/tmp/kzg-option-house-v168-dist-desktop.png`, `/tmp/kzg-option-house-v168-handoff-desktop.png`, `/tmp/kzg-option-house-v168-dist-mobile.png`, `/tmp/kzg-option-house-v168-handoff-mobile.png`, and `/tmp/kzg-option-house-v168-export.png`.
- Deploy: Deployed to production. Production site <https://kzg-option-house.netlify.app/>; unique deploy <https://6a16acc1dbce120ee54cac50--kzg-option-house.netlify.app/>; deploy id `6a16acc1dbce120ee54cac50`. Live `app.js` verifies `UI_VERSION = "1.68"`; live `/data/index.json` and `/assets/kzg-pack.js` both return `404`; live Playwright desktop/phone checks match local verification, and PNG export remains `1,482,138` bytes.
- Next: v1.69 should stay on Web and connect the symbol lens and momentum list back into capital anchors/rotation rows, reducing the remaining break after a user finishes reading RGTI. iOS companion remains `0.7`; the next default iOS sync is around Web v1.72.

## v1.67 - 2026-05-27 16:15 Asia/Shanghai - iOS bucket-to-rotation handoff sync

中文:

- 改动: 稠密版本 `1.67` 不改 Web 公开站，不部署生产；本轮把 Web v1.66 的 bucket-to-rotation 接力桥同步到原生 iOS 伴生端 `0.7`。`ios/KZGOptionHouse/KZGOptionHouse/DashboardView.swift` 顶部 checkpoint 改为 Web `1.66`、iOS `0.7`、bridge/handoff；Header 显示 `iOS companion 0.7`；`IntradayCard` 与 `RotationCard` 之间新增 `BucketRotationHandoffCard`，把压力桶、CP 两端、扩散率和领头标的压成手机原生卡。
- 体验: iOS 现在也能顺着 Web 的开放历史读盘路径阅读：先看日内压力桶 `15:00 +30%`，再看 CP 两端 `10:30 / 15:00`，最后看 `5 / 2` 扩散率和 `RGTI`，然后进入轮动象限。它是公开历史层的手机导流，不是实时权限、解锁、订阅、支付、provider 或 API 路线。
- 改动文件: `ios/KZGOptionHouse/KZGOptionHouse/DashboardView.swift`、`docs/CHANGELOG.md`、`docs/CHANGLOG_OPTION_TERMINAL.md`、`docs/IOS_COMPANION_PLAN.md`、`docs/DENSE_VERSIONING.md`、`docs/HANDOFF_FOR_OTHER_CODEX.md`、`docs/MASSIVE_REALTIME_PRODUCT_PLAN.md`、`docs/PLUGIN_SERVICE_STATUS.md`、`docs/CHANGLOG_OPTION_TERMINAL_MINDMAP.opml`。
- 验证: XcodeBuildMCP `session_show_defaults` 确认当前 profile `kzg-option-house-ios` 指向 `/Users/fangbao/kzg-options-minute-site/ios/KZGOptionHouse/KZGOptionHouse.xcodeproj`、scheme `KZG Option House`、simulator `iPhone 17 Pro`。`build_run_sim` 仍被本机 simulator destination 阻塞，错误为无法找到 destination `{ platform:iOS Simulator, id:9DAFEA29-80F2-4D94-BE75-C0106CE8D97E }`。Fallback `xcrun --sdk iphonesimulator swiftc -typecheck ios/KZGOptionHouse/KZGOptionHouse/*.swift -target arm64-apple-ios17.0-simulator` 通过；`git diff --check -- ios/KZGOptionHouse/KZGOptionHouse/DashboardView.swift` 通过；iOS Swift 源码公开风险词扫描无命中。Web 源码本轮未改，v1.66 PNG 导出证明 `1,482,138` bytes 继续作为最近 Web 证明。
- 部署: 本轮不部署生产，不做 TestFlight、App Store、签名 team、开发者账号、付费账号或任何真实凭证动作。生产仍是 v1.63，生产站 <https://kzg-option-house.netlify.app/>，唯一部署 <https://6a168b3efbed57514f5fc1ea--kzg-option-house.netlify.app/>。
- 下一步: v1.68 回到 Web，继续压缩开放历史下半区的手机高度和连续读盘，优先处理 bucket bars、rotation lists、symbol momentum 的节奏；iOS 下一次默认同步约在 Web v1.72，除非 Fangbao 提前要求。

English:

- Change: Dense `1.67` does not change the public Web app or deploy production. This round syncs Web v1.66's bucket-to-rotation handoff bridge into native iOS companion `0.7`. `ios/KZGOptionHouse/KZGOptionHouse/DashboardView.swift` updates the checkpoint strip to Web `1.66`, iOS `0.7`, bridge/handoff; the Header shows `iOS companion 0.7`; `BucketRotationHandoffCard` is added between `IntradayCard` and `RotationCard`, compressing pressure bucket, CP extremes, breadth, and leading symbol into a native phone card.
- Experience: iOS now follows the same open-history reading path as Web: read the intraday pressure bucket `15:00 +30%`, then CP extremes `10:30 / 15:00`, then `5 / 2` breadth and `RGTI`, then enter the rotation quadrant. This is phone guidance for the open historical layer, not realtime entitlement, unlock, subscription, payment, provider, or API routing.
- Changed files: `ios/KZGOptionHouse/KZGOptionHouse/DashboardView.swift`, `docs/CHANGELOG.md`, `docs/CHANGLOG_OPTION_TERMINAL.md`, `docs/IOS_COMPANION_PLAN.md`, `docs/DENSE_VERSIONING.md`, `docs/HANDOFF_FOR_OTHER_CODEX.md`, `docs/MASSIVE_REALTIME_PRODUCT_PLAN.md`, `docs/PLUGIN_SERVICE_STATUS.md`, and `docs/CHANGLOG_OPTION_TERMINAL_MINDMAP.opml`.
- Verification: XcodeBuildMCP `session_show_defaults` confirmed current profile `kzg-option-house-ios` points to `/Users/fangbao/kzg-options-minute-site/ios/KZGOptionHouse/KZGOptionHouse.xcodeproj`, scheme `KZG Option House`, simulator `iPhone 17 Pro`. `build_run_sim` is still blocked by the local simulator destination: unable to find destination `{ platform:iOS Simulator, id:9DAFEA29-80F2-4D94-BE75-C0106CE8D97E }`. Fallback `xcrun --sdk iphonesimulator swiftc -typecheck ios/KZGOptionHouse/KZGOptionHouse/*.swift -target arm64-apple-ios17.0-simulator` passed; `git diff --check -- ios/KZGOptionHouse/KZGOptionHouse/DashboardView.swift` passed; public-risk scan across iOS Swift source had no matches. Web source was not changed this round, so the v1.66 PNG proof at `1,482,138` bytes remains the latest Web PNG proof.
- Deploy: Not deployed. No TestFlight, App Store, signing-team, developer-account, paid account, or credential action happened. Production remains v1.63 at <https://kzg-option-house.netlify.app/>, unique deploy <https://6a168b3efbed57514f5fc1ea--kzg-option-house.netlify.app/>.
- Next: v1.68 should return to Web and keep compressing lower open-history phone height and reading continuity, especially bucket bars, rotation lists, and symbol momentum. The next default iOS sync is around Web v1.72 unless Fangbao asks earlier.

## v1.66 - 2026-05-27 16:04 Asia/Shanghai - bucket-to-rotation handoff bridge

中文:

- 改动: 公开 Web UI 从 `1.65` 提到 `1.66`。本轮不部署生产，不改权限边界，不新增付费墙，不接真实 API。`public/index.html` 在日内 bucket 风险卡与 bucket 柱状图之间加入 `#bucketRotationBridge`。`public/app.js` 新增 `bucketRotationBridge()`，把最大成交冲击、CP 两端和标的轮动扩散压成一条“分钟压力接力”，并复用 v1.65 的 `scrollToSector()`，点击 `进入轮动象限` 会滚到 `symbolRotation` 并触发目标面板聚焦。`public/styles.css` 新增桌面/手机和深色样式，确保桥不横向溢出。
- 体验: Fangbao 要求底部开放历史层不要像一堆复杂模块。本轮处理的是 `日内节奏 vs 20日均值` 到 `标的轮动扩散` 之间的断点：先读 `15:00 +30%` 的压力桶，再读 `Call 10:30 / Put 15:00` 的 CP 两端，最后读 `15 / 12`、`RGTI` 的扩散方向。它是开放历史层的读盘导流，不是 unlock、subscription、付费暗示、realtime entitlement 或 provider 入口。
- 改动文件: `public/index.html`、`public/app.js`、`public/styles.css`、`docs/CHANGELOG.md`、`docs/CHANGLOG_OPTION_TERMINAL.md`、`docs/DENSE_VERSIONING.md`、`docs/HANDOFF_FOR_OTHER_CODEX.md`、`docs/MASSIVE_REALTIME_PRODUCT_PLAN.md`、`docs/PLUGIN_SERVICE_STATUS.md`、`docs/CHANGLOG_OPTION_TERMINAL_MINDMAP.opml`。
- 验证: `node --check public/app.js` 通过；`git diff --check -- public/index.html public/app.js public/styles.css` 通过；公开风险词扫描 `public/app.js public/index.html public/styles.css` 无命中。`python3 scripts/build_payload.py` 生成 `505` 天 payload，latest `2026-05-22`，analytics days `505`，analytics symbols `98`，pack asset `kzg-frame-ee3731126f01.js`；`python3 scripts/per_day_to_dist.py` 返回 `copied=505`。Playwright 对 dist `http://127.0.0.1:4207/` 验证桌面 `1440x1100`：`sourcePath` 为 `1.66 · 505/505 complete`，bridge 1 个，三格接力，jump 1，点击后 `.rotation-panel.history-scroll-focus = 1`，横向溢出 0，旧 `.is-blurred`/`.is-locked` 0，future gate 1，console issue 0。Playwright 对 dist 手机 `390x844` 验证同样通过：bridge 1 个，三格接力，jump 1，点击后 rotation focus 1，横向溢出 0，旧 blur/lock 0，future gate 1，console issue 0。PNG 导出保持可用，大小 `1,482,138` bytes。截图证据: `/tmp/kzg-option-house-v166-dist-desktop.png`、`/tmp/kzg-option-house-v166-bridge-desktop-dist.png`、`/tmp/kzg-option-house-v166-dist-mobile.png`、`/tmp/kzg-option-house-v166-bridge-mobile.png`、`/tmp/kzg-option-house-v166-export.png`。
- 部署: 本轮不部署生产。生产仍是 v1.63，生产站 <https://kzg-option-house.netlify.app/>，唯一部署 <https://6a168b3efbed57514f5fc1ea--kzg-option-house.netlify.app/>。不部署原因：v1.64-v1.66 是本地开放历史层阅读节奏增强；未发现公开风险修复需求。
- 下一步: v1.67 到达默认 iOS 同步点，应优先同步 iOS companion 或明确记录延期理由；Web 侧继续压缩开放历史下半区的手机高度和图表节奏，尤其是 bucket bars、rotation lists、symbol momentum 之间的连续读盘关系。仍不接真实 key、不做真实升级、不花钱、不提交 raw data。

English:

- Change: Public Web UI moves from `1.65` to `1.66`. This round does not deploy production, change access boundaries, add a paywall, or connect a real API. `public/index.html` adds `#bucketRotationBridge` between the intraday bucket risk card and bucket bars. `public/app.js` adds `bucketRotationBridge()`, compressing the largest volume shock, CP extremes, and rotation breadth into a `minute pressure handoff`, and reuses v1.65 `scrollToSector()` so tapping `Enter rotation quadrant` scrolls to `symbolRotation` and focuses the target panel. `public/styles.css` adds desktop, phone, and dark-mode styling so the bridge does not overflow horizontally.
- Experience: Fangbao wanted the lower open-history layer to stop feeling like a stack of complex modules. This round fixes the break between `Intraday rhythm vs 20D` and `Symbol rotation breadth`: read the `15:00 +30%` pressure bucket first, then the `Call 10:30 / Put 15:00` CP extremes, then the `15 / 12` breadth and `RGTI` rotation direction. This is open-history reading guidance, not an unlock, subscription, paid hint, realtime entitlement, or provider entry.
- Changed files: `public/index.html`, `public/app.js`, `public/styles.css`, `docs/CHANGELOG.md`, `docs/CHANGLOG_OPTION_TERMINAL.md`, `docs/DENSE_VERSIONING.md`, `docs/HANDOFF_FOR_OTHER_CODEX.md`, `docs/MASSIVE_REALTIME_PRODUCT_PLAN.md`, `docs/PLUGIN_SERVICE_STATUS.md`, and `docs/CHANGLOG_OPTION_TERMINAL_MINDMAP.opml`.
- Verification: `node --check public/app.js` passed; `git diff --check -- public/index.html public/app.js public/styles.css` passed; public-risk scan across `public/app.js public/index.html public/styles.css` had no matches. `python3 scripts/build_payload.py` produced a `505`-day payload, latest `2026-05-22`, analytics days `505`, analytics symbols `98`, pack asset `kzg-frame-ee3731126f01.js`; `python3 scripts/per_day_to_dist.py` returned `copied=505`. Playwright against dist `http://127.0.0.1:4207/` verified desktop `1440x1100`: `sourcePath` showed `1.66 · 505/505 complete`, bridge 1, three bridge cells, jump 1, `.rotation-panel.history-scroll-focus = 1` after click, horizontal overflow 0, old `.is-blurred`/`.is-locked` 0, future gate 1, and console issues 0. Playwright against dist phone `390x844` also passed: bridge 1, three bridge cells, jump 1, rotation focus 1 after click, horizontal overflow 0, old blur/lock 0, future gate 1, console issues 0. PNG export remains working at `1,482,138` bytes. Screenshot evidence: `/tmp/kzg-option-house-v166-dist-desktop.png`, `/tmp/kzg-option-house-v166-bridge-desktop-dist.png`, `/tmp/kzg-option-house-v166-dist-mobile.png`, `/tmp/kzg-option-house-v166-bridge-mobile.png`, and `/tmp/kzg-option-house-v166-export.png`.
- Deploy: Not deployed this round. Production remains v1.63 at <https://kzg-option-house.netlify.app/>, unique deploy <https://6a168b3efbed57514f5fc1ea--kzg-option-house.netlify.app/>. Reason: v1.64-v1.66 are local open-history reading-rhythm improvements with no public-risk fix required.
- Next: v1.67 hits the default iOS sync checkpoint and should either sync the iOS companion or explicitly record deferral. On Web, keep compressing lower open-history phone height and chart rhythm, especially continuity between bucket bars, rotation lists, and symbol momentum. Still do not connect real keys, perform real upgrades, spend money, or commit raw data.

## v1.65 - 2026-05-27 15:44 Asia/Shanghai - story-to-panel focus handoff

中文:

- 改动: 公开 Web UI 从 `1.64` 提到 `1.65`。本轮不部署生产，不改权限边界，不新增付费墙，不接真实 API。`public/app.js` 把 `[data-scroll-sector]` 点击逻辑从直接 `scrollIntoView()` 升级为 `scrollToSector()`，点击 v1.64 四拍故事条或路径卡时，先定位目标节点，再找到其外层 `.panel`，移除旧焦点，给目标面板临时加 `.history-scroll-focus`，滚动到目标区并在 1.8 秒后自动清掉聚焦态。`public/styles.css` 给 `.panel` 增加 `scroll-margin-top: 88px`，并新增浅色/深色两套 `history-scroll-focus` 动画。
- 体验: v1.64 解决“进入历史层先看什么”，但用户点完故事条后仍可能在长页面里失去定位感。v1.65 解决“我刚点的是哪里、现在落到了哪块”的问题：点击 `量变` 会落到跨日趋势，目标趋势面板出现短暂金色聚焦；点击手机端 `扩散` 会落到轮动象限，面板短暂高亮。这个反馈只服务开放历史阅读，不是 lock、unlock、subscription、付费暗示，也不影响 realtime reserve 的唯一允许 blur。
- 改动文件: `public/app.js`、`public/styles.css`、`docs/CHANGELOG.md`、`docs/CHANGLOG_OPTION_TERMINAL.md`、`docs/DENSE_VERSIONING.md`、`docs/HANDOFF_FOR_OTHER_CODEX.md`、`docs/MASSIVE_REALTIME_PRODUCT_PLAN.md`、`docs/PLUGIN_SERVICE_STATUS.md`、`docs/CHANGLOG_OPTION_TERMINAL_MINDMAP.opml`。
- 验证: `node --check public/app.js` 通过；`git diff --check -- public/app.js public/styles.css` 通过；公开风险词扫描 `public/app.js public/index.html public/styles.css` 无命中。`python3 scripts/build_payload.py` 生成 `505` 天 payload，latest `2026-05-22`，analytics days `505`，analytics symbols `98`，pack asset `kzg-frame-f4c56892294d.js`；`python3 scripts/per_day_to_dist.py` 返回 `copied=505`。Browser 本地打开 `http://127.0.0.1:4204/`，确认标题、`1.65`、story cards 4、path buttons 4、future realtime gated 1、旧 blur/lock 0、横向溢出 0、console issue 0；点击故事卡后目标 `.panel.history-scroll-focus` 存在。Playwright 对 dist `http://127.0.0.1:4205/` 验证桌面 `1440x1100` 和手机 `390x844`：页面非空、标题正确、story cards 4、桌面点击 `trendChart` 后 `.trend-panel.history-scroll-focus` 为 1、手机点击 `symbolRotation` 后 `.rotation-panel.history-scroll-focus` 为 1、横向溢出 0、旧 `.is-blurred`/`.is-locked` 0、console issue 0；最终构建后手机 smoke 仍显示 `1.65 · 505/505 complete`、story cards 4、rotation focus 1、横向溢出 0。PNG 导出保持不变，文件 `/tmp/kzg-option-house-v165-export.png`，大小 `1,482,138` bytes。截图证据: `/tmp/kzg-option-house-v165-dist-desktop.png`、`/tmp/kzg-option-house-v165-focus-desktop.png`、`/tmp/kzg-option-house-v165-dist-mobile.png`、`/tmp/kzg-option-house-v165-focus-mobile.png`、`/tmp/kzg-option-house-v165-export.png`。
- 部署: 本轮不部署生产。生产仍是 v1.63，生产站 <https://kzg-option-house.netlify.app/>，唯一部署 <https://6a168b3efbed57514f5fc1ea--kzg-option-house.netlify.app/>。不部署原因：v1.63 刚完成生产部署，v1.64/v1.65 是本地开放历史层阅读和交互节奏增强；未发现公开风险修复需求。
- 下一步: v1.66 应继续把开放历史下半区做得更像一个可读产品，而不是一堆重型模块：优先压缩 bucket risk 到 symbol rotation 之间的转场、检查手机端下拉后的图表高度和文字节奏；仍不接真实 key、不做真实升级、不花钱、不提交 raw data。iOS 下次默认同步仍在 Web v1.67 左右，除非 Fangbao 提前要求。

English:

- Change: Public Web UI moves from `1.64` to `1.65`. This round does not deploy production, change access boundaries, add a paywall, or connect a real API. `public/app.js` upgrades `[data-scroll-sector]` clicks from direct `scrollIntoView()` to `scrollToSector()`: when a v1.64 story card or path card is clicked, it finds the target, resolves the enclosing `.panel`, clears any previous focus, adds `.history-scroll-focus`, scrolls to the target, and removes the focus state after 1.8 seconds. `public/styles.css` adds `scroll-margin-top: 88px` to `.panel` and adds light/dark `history-scroll-focus` animations.
- Experience: v1.64 answered what to read first inside the historical layer, but after tapping a story card users could still lose orientation in the long page. v1.65 answers where the click landed: tapping `Drift` lands on cross-day trend with a short gold focus pulse; tapping `Breadth` on mobile lands on the rotation panel with a short highlight. This is only an open-history reading aid. It is not a lock, unlock, subscription, paid hint, or change to the realtime reserve's only allowed blur.
- Changed files: `public/app.js`, `public/styles.css`, `docs/CHANGELOG.md`, `docs/CHANGLOG_OPTION_TERMINAL.md`, `docs/DENSE_VERSIONING.md`, `docs/HANDOFF_FOR_OTHER_CODEX.md`, `docs/MASSIVE_REALTIME_PRODUCT_PLAN.md`, `docs/PLUGIN_SERVICE_STATUS.md`, and `docs/CHANGLOG_OPTION_TERMINAL_MINDMAP.opml`.
- Verification: `node --check public/app.js` passed; `git diff --check -- public/app.js public/styles.css` passed; public-risk scan across `public/app.js public/index.html public/styles.css` had no matches. `python3 scripts/build_payload.py` produced a `505`-day payload, latest `2026-05-22`, analytics days `505`, analytics symbols `98`, pack asset `kzg-frame-f4c56892294d.js`; `python3 scripts/per_day_to_dist.py` returned `copied=505`. Browser opened local `http://127.0.0.1:4204/` and confirmed title, `1.65`, 4 story cards, 4 path buttons, future realtime gated 1, old blur/lock 0, horizontal overflow 0, and 0 console issues; after clicking a story card, the target `.panel.history-scroll-focus` exists. Playwright against dist `http://127.0.0.1:4205/` verified desktop `1440x1100` and phone `390x844`: page not blank, title correct, story cards 4, desktop click to `trendChart` gives `.trend-panel.history-scroll-focus = 1`, phone click to `symbolRotation` gives `.rotation-panel.history-scroll-focus = 1`, horizontal overflow 0, old `.is-blurred`/`.is-locked` 0, and console issues 0; post-build phone smoke still showed `1.65 · 505/505 complete`, story cards 4, rotation focus 1, and horizontal overflow 0. PNG export stayed unchanged at `/tmp/kzg-option-house-v165-export.png`, size `1,482,138` bytes. Screenshot evidence: `/tmp/kzg-option-house-v165-dist-desktop.png`, `/tmp/kzg-option-house-v165-focus-desktop.png`, `/tmp/kzg-option-house-v165-dist-mobile.png`, `/tmp/kzg-option-house-v165-focus-mobile.png`, and `/tmp/kzg-option-house-v165-export.png`.
- Deploy: Not deployed this round. Production remains v1.63 at <https://kzg-option-house.netlify.app/>, unique deploy <https://6a168b3efbed57514f5fc1ea--kzg-option-house.netlify.app/>. Reason: v1.63 was just deployed, and v1.64/v1.65 are local open-history readability and interaction-rhythm improvements with no public-risk fix required.
- Next: v1.66 should keep turning the lower open-history area into a readable product rather than a stack of heavy modules. Prioritize the transition between bucket risk and symbol rotation, then phone chart height and text rhythm. Still do not connect real keys, perform real upgrades, spend money, or commit raw data. The next default iOS sync remains around Web v1.67 unless Fangbao asks earlier.

## v1.64 - 2026-05-27 14:40 Asia/Shanghai - open-history four-beat story strip

中文:

- 改动: 公开 Web UI 从 `1.63` 提到 `1.64`。本轮不部署生产，不改权限边界，不新增付费墙。`public/app.js` 在 v1.63 的 `historyLayerPath()` 下面新增 `historyStoryStrip()`，把开放历史层再压成“开放读盘四拍”：`量变`、`压力分钟`、`扩散`、`权利金锚点`。这四拍仍是公开可见的历史分钟聚合读盘，不是 realtime API、不是 paid subscription、不是 unlock 入口。
- 体验: v1.63 已经有 4 个滚动入口，但用户仍会先看到较密的路径和下方重型图表。本轮新增一条更像读盘故事的简化层：先看 `02/27 -> 05/22 -2.5%` 的跨日量变，再看 `09:30 1,147.8万张` 压力分钟，再看 `15 / 12` 的量价同升/同步降温扩散，最后用 `SPY $2.1B` 作为权利金资金锚点。首次检查发现原先取“权利金增速领头”会把小金额高增速标的误写成资金锚点，所以改为 `premiumNotional` 最大的真实金额锚点。
- 改动文件: `public/app.js`、`public/styles.css`、`docs/CHANGELOG.md`、`docs/CHANGLOG_OPTION_TERMINAL.md`、`docs/DENSE_VERSIONING.md`、`docs/HANDOFF_FOR_OTHER_CODEX.md`、`docs/MASSIVE_REALTIME_PRODUCT_PLAN.md`、`docs/PLUGIN_SERVICE_STATUS.md`、`docs/CHANGLOG_OPTION_TERMINAL_MINDMAP.opml`。
- 验证: `node --check public/app.js` 通过；`git diff --check -- public/app.js public/styles.css` 通过；公开风险词扫描 `public/app.js public/index.html public/styles.css` 无命中。`python3 scripts/build_payload.py` 生成 `505` 天 payload，latest `2026-05-22`，analytics days `505`，analytics symbols `98`，pack asset `kzg-frame-79fe0b91de61.js`；`python3 scripts/per_day_to_dist.py` 返回 `copied=505`。Browser 本地打开 `http://127.0.0.1:4202/`，确认标题、`1.64`、`historyStoryStrip`、4 张 story card、4 个 path button、旧 `.is-blurred` 0、future realtime gated 1、公开风险词 false。Playwright 对 dist `http://127.0.0.1:4203/` 验证桌面 `1440x1100` 和手机 `390x844`：story cards 4、path buttons 4、横向溢出 0、旧 `.is-blurred` 0、future realtime gated 1；手机 `#sourcePath` 显示 `1.64`；PNG 导出成功，大小仍为 `1,482,138` bytes。截图证据: `/tmp/kzg-option-house-v164-dist-desktop.png`、`/tmp/kzg-option-house-v164-story-desktop.png`、`/tmp/kzg-option-house-v164-dist-mobile.png`、`/tmp/kzg-option-house-v164-story-mobile.png`、`/tmp/kzg-option-house-v164-export.png`。
- 部署: 本轮不部署生产。生产仍是 v1.63，生产站 <https://kzg-option-house.netlify.app/>，唯一部署 <https://6a168b3efbed57514f5fc1ea--kzg-option-house.netlify.app/>。不部署原因：v1.63 刚完成生产部署，v1.64 是本地公开历史层阅读简化；未发现公开风险修复需求。
- 下一步: v1.65 继续压缩历史层密度，重点处理 story strip 到轮动象限、bucket risk、symbol momentum 的滚动节奏；仍不接真实 key、不做真实升级、不花钱、不提交 raw data。iOS 下次默认同步仍在 Web v1.67 左右，除非 Fangbao 提前要求。

English:

- Change: Public Web UI moves from `1.63` to `1.64`. This round does not deploy production, change access boundaries, or add a paywall. `public/app.js` adds `historyStoryStrip()` below v1.63's `historyLayerPath()`, compressing the open historical layer into four reading beats: `Drift`, `Pressure minute`, `Breadth`, and `Premium anchor`. These beats remain public historical minute-aggregate reading aids, not a realtime API, paid subscription, or unlock entry.
- Experience: v1.63 already added 4 scroll targets, but users still hit a dense path and heavy chart stack. v1.64 adds a simpler story layer: first read the `02/27 -> 05/22 -2.5%` cross-day drift, then `09:30 1,147.8万` pressure minute, then `15 / 12` breadth between warming and cooling symbols, then `SPY $2.1B` as the premium-notional capital anchor. Initial QA showed that using the premium-growth leader could turn a small high-growth name into the money anchor, so the card now uses the largest `premiumNotional` anchor instead.
- Changed files: `public/app.js`, `public/styles.css`, `docs/CHANGELOG.md`, `docs/CHANGLOG_OPTION_TERMINAL.md`, `docs/DENSE_VERSIONING.md`, `docs/HANDOFF_FOR_OTHER_CODEX.md`, `docs/MASSIVE_REALTIME_PRODUCT_PLAN.md`, `docs/PLUGIN_SERVICE_STATUS.md`, and `docs/CHANGLOG_OPTION_TERMINAL_MINDMAP.opml`.
- Verification: `node --check public/app.js` passed; `git diff --check -- public/app.js public/styles.css` passed; public-risk scan across `public/app.js public/index.html public/styles.css` had no matches. `python3 scripts/build_payload.py` produced a `505`-day payload, latest `2026-05-22`, analytics days `505`, analytics symbols `98`, pack asset `kzg-frame-79fe0b91de61.js`; `python3 scripts/per_day_to_dist.py` returned `copied=505`. Browser opened local `http://127.0.0.1:4202/` and confirmed title, `1.64`, `historyStoryStrip`, 4 story cards, 4 path buttons, old `.is-blurred` 0, future realtime gated 1, and public-risk false. Playwright against dist `http://127.0.0.1:4203/` verified desktop `1440x1100` and phone `390x844`: story cards 4, path buttons 4, horizontal overflow 0, old `.is-blurred` 0, future realtime gated 1; phone `#sourcePath` shows `1.64`; PNG export succeeded and stayed at `1,482,138` bytes. Screenshot evidence: `/tmp/kzg-option-house-v164-dist-desktop.png`, `/tmp/kzg-option-house-v164-story-desktop.png`, `/tmp/kzg-option-house-v164-dist-mobile.png`, `/tmp/kzg-option-house-v164-story-mobile.png`, and `/tmp/kzg-option-house-v164-export.png`.
- Deploy: Not deployed this round. Production remains v1.63 at <https://kzg-option-house.netlify.app/>, unique deploy <https://6a168b3efbed57514f5fc1ea--kzg-option-house.netlify.app/>. Reason: v1.63 was just deployed, and v1.64 is a local open-history readability simplification with no public-risk fix required.
- Next: v1.65 should keep reducing historical-layer density, especially the scroll rhythm from the story strip into rotation quadrant, bucket risk, and symbol momentum. Still do not connect real keys, perform real upgrades, spend money, or commit raw data. The next default iOS sync remains around Web v1.67 unless Fangbao asks earlier.

## v1.63 - 2026-05-27 14:12 Asia/Shanghai - open historical path router and production deploy

中文:

- 改动: 公开 Web UI 从 `1.62` 提到 `1.63`。本轮继续遵守 Fangbao 最新边界：当前生成分钟能力全部开放，历史、趋势、轮动、事件队列、PNG 导出不 blur、不 lock、不 paywall；只有未来 realtime option-flow tape/reserve 保留预留式 blur。`public/app.js` 在 `renderHistorySectorIntro()` 后加入 `historyLayerPath()`，把实时预留区后的开放历史层压成 4 个明确入口：`跨日趋势`、`日内桶`、`轮动象限`、`标的聚焦`。每个入口都是真实按钮，点击滚到 `trendChart`、`bucketProfile`、`symbolRotation`、`symbolFocus`，让中段未来实时故事自然落到下面免费的 505 日历史读盘层。
- 体验: 桌面上新增 “开放历史路径 / 四步进入真正可读的 505 日层” 横向路径条，分别展示 `02/27 -> 05/22 -2.5%`、`09:30 1,147.8万张峰值`、`15 / 12 RGTI 量价方向`、`RGTI $122.7M`。手机端原先两列会压缩日期和金额，本轮补了 520px 以下单列布局，并允许关键值换行，避免把核心信息藏进省略号。
- 改动文件: `public/app.js`、`public/styles.css`、`docs/CHANGELOG.md`、`docs/CHANGLOG_OPTION_TERMINAL.md`、`docs/DENSE_VERSIONING.md`、`docs/HANDOFF_FOR_OTHER_CODEX.md`、`docs/MASSIVE_REALTIME_PRODUCT_PLAN.md`、`docs/PLUGIN_SERVICE_STATUS.md`、`docs/CHANGLOG_OPTION_TERMINAL_MINDMAP.opml`。
- 验证: `node --check public/app.js` 通过；`git diff --check -- public/app.js public/styles.css` 通过；公开风险词扫描 `public/app.js public/index.html public/styles.css` 无命中。`python3 scripts/build_payload.py` 生成 `505` 天 payload，latest `2026-05-22`，analytics days `505`，analytics symbols `98`，pack asset `kzg-frame-c5239c6574b9.js`；`python3 scripts/per_day_to_dist.py` 返回 `copied=505`。Browser 本地打开 `http://127.0.0.1:4200/`，确认标题、history path、4 个按钮、4 个目标、旧锁层 0、旧 `.is-blurred` 0、横向溢出 0、console issue 0。Playwright 对 dist `http://127.0.0.1:4201/` 验证桌面 `1440x1100` 和手机 `390x844`：4 个 path 目标存在，桌面 5 列路径条，手机 1 列路径卡，横向溢出 0，旧锁层 0，旧 `.is-blurred` 0，future blur 仅在 `.terminal-table-body.is-realtime-gated`。PNG 导出成功，大小 `1,482,138` bytes。
- 部署: 已部署生产。生产站 <https://kzg-option-house.netlify.app/>；唯一部署 <https://6a168b3efbed57514f5fc1ea--kzg-option-house.netlify.app/>；deploy id `6a168b3efbed57514f5fc1ea`。线上 `app.js` 验证 `UI_VERSION = "1.63"`；线上 `/data/index.json` 和 `/assets/kzg-pack.js` 都是 `404`；线上 Browser 与 Playwright 验证 desktop/mobile 无 console issue、无横向溢出、旧 blur/lock 为 0，PNG 导出仍为 `1,482,138` bytes。
- 下一步: v1.64 继续做开放历史层的读盘简化，重点减少下方密集模块的阅读负担，把最强的量价齐升、降温、压力分钟、权利金锚点做成更自然的故事线；仍不接真实 key、不做真实升级、不花钱、不提交 raw data。iOS 下次默认同步仍在 Web v1.67 左右，除非 Fangbao 提前要求。

English:

- Change: Public Web UI moves from `1.62` to `1.63`. Fangbao's latest boundary stays intact: current generated-minute capabilities remain fully open; history, trends, rotation, event queues, and PNG export are not blurred, locked, or paywalled; only the future realtime option-flow tape/reserve keeps a reserve-style blur. `public/app.js` adds `historyLayerPath()` after `renderHistorySectorIntro()`, turning the open historical layer after the realtime reserve into four clear entry buttons: `Cross-day`, `Intraday buckets`, `Rotation map`, and `Symbol lens`. Each button scrolls to `trendChart`, `bucketProfile`, `symbolRotation`, and `symbolFocus`, so the future realtime story lands cleanly into the free 505-session historical reading layer.
- Experience: Desktop now has an "Open history path / Four steps into the readable 505-session layer" rail showing `02/27 -> 05/22 -2.5%`, `09:30 1,147.8万 peak`, `15 / 12 RGTI direction`, and `RGTI $122.7M`. On phones the earlier two-column path could compress dates and money values, so v1.63 switches below 520px to a single-column layout and lets key values wrap instead of hiding them behind ellipses.
- Changed files: `public/app.js`, `public/styles.css`, `docs/CHANGELOG.md`, `docs/CHANGLOG_OPTION_TERMINAL.md`, `docs/DENSE_VERSIONING.md`, `docs/HANDOFF_FOR_OTHER_CODEX.md`, `docs/MASSIVE_REALTIME_PRODUCT_PLAN.md`, `docs/PLUGIN_SERVICE_STATUS.md`, and `docs/CHANGLOG_OPTION_TERMINAL_MINDMAP.opml`.
- Verification: `node --check public/app.js` passed; `git diff --check -- public/app.js public/styles.css` passed; public-risk scan across `public/app.js public/index.html public/styles.css` had no matches. `python3 scripts/build_payload.py` produced a `505`-day payload, latest `2026-05-22`, analytics days `505`, analytics symbols `98`, pack asset `kzg-frame-c5239c6574b9.js`; `python3 scripts/per_day_to_dist.py` returned `copied=505`. Browser opened local `http://127.0.0.1:4200/` and confirmed title, history path, 4 buttons, 4 targets, 0 old lock layers, 0 old `.is-blurred` nodes, 0 horizontal overflow, and 0 console issues. Playwright against dist `http://127.0.0.1:4201/` verified desktop `1440x1100` and phone `390x844`: all 4 path targets exist, desktop uses a 5-column path rail, phone uses 1-column path cards, horizontal overflow 0, old locks 0, old `.is-blurred` 0, future blur only on `.terminal-table-body.is-realtime-gated`. PNG export succeeded at `1,482,138` bytes.
- Deploy: Deployed to production. Production site <https://kzg-option-house.netlify.app/>; unique deploy <https://6a168b3efbed57514f5fc1ea--kzg-option-house.netlify.app/>; deploy id `6a168b3efbed57514f5fc1ea`. Production `app.js` verifies `UI_VERSION = "1.63"`; production `/data/index.json` and `/assets/kzg-pack.js` both return `404`; production Browser and Playwright checks pass on desktop/mobile with no console issues, no horizontal overflow, 0 old blur/lock nodes, and PNG export still `1,482,138` bytes.
- Next: v1.64 should keep simplifying the open historical layer, especially making volume-premium warming, cooling, pressure minutes, and premium anchors read as a more natural story. Still do not connect real keys, perform real upgrades, spend money, or commit raw data. The next default iOS sync remains around Web v1.67 unless Fangbao asks earlier.

## v1.62 - 2026-05-27 13:40 Asia/Shanghai - realtime-to-open-history handoff and iOS 0.6 sync

中文:

- 改动: 公开 Web UI 从 `1.61` 提到 `1.62`。本轮不改变 Fangbao 最新公开边界：当前生成分钟能力继续开放，历史、趋势、轮动、事件队列、PNG 导出不 blur、不 lock、不 paywall；只有未来 realtime option-flow tape/reserve 允许保留预留式 blur。`public/app.js` 新增 `realtimeHistoryHandoff()`，把中段未来实时席位和下方免费历史层之间接上一条“Open layer handoff / 实时预留到这里，下方继续免费读历史”的桥。它明确告诉用户：实时区是未来席位，下面的 505 日历史窗口继续开放，并把量价同升、压力分钟、降温队列这些当前能读的历史能力顺手带出来。
- 体验: v1.61 解决手机 realtime reserve 过长，v1.62 解决“实时区结束后为什么继续往下看”的问题。handoff 桥在桌面上是说明栏加四个短格：505 日开放历史、RGTI 量价同升、09:30 压力分钟、SPY 同步降温；手机上压成两列短格，不再制造大段说明。`symbolRotationRows()` 加了 fallback，让某些开发状态没有 symbol history 时也不会把轮动接力层渲染成空白。
- iOS: 原生 SwiftUI 伴生从 `0.5` 同步到 `0.6`。`DashboardView.swift` 的 checkpoint strip 显示 Web `1.62`、iOS `0.6`、Flow `Future`；新增 `FlowRouterCard`、`RouterGateTile`、`TapeChip`，把 Web 的 Flow Router 逻辑同步到手机原生界面。iOS 仍不放真实 API key、不放供应商名、不放支付、域名、价格、注册或账号路线。
- 验证: `node --check public/app.js` 通过；`xcrun --sdk iphonesimulator swiftc -typecheck ios/KZGOptionHouse/KZGOptionHouse/*.swift -target arm64-apple-ios17.0-simulator` 通过；`git diff --check -- public/app.js public/styles.css ios/KZGOptionHouse/KZGOptionHouse/DashboardView.swift` 通过；公开风险词扫描 `public/app.js public/index.html public/styles.css ios/KZGOptionHouse/KZGOptionHouse/*.swift` 无命中。`python3 scripts/build_payload.py` 生成 `505` 天 payload，latest `2026-05-22`，analytics days `505`，analytics symbols `98`，pack asset `kzg-frame-6b392e1504a6.js`；`python3 scripts/per_day_to_dist.py` 返回 `copied=505`。
- QA: Browser 打开本地 public `http://127.0.0.1:4198/`，确认 source 为 `23_DATA_期权分钟_Minute · 1.62 · 505/505 complete`，Flow Router 存在、handoff 存在、handoff 显示 505 日，4 个 gate、16 格 tape，旧锁层 0、旧 `.is-blurred` 0、横向溢出 0、console issue 0、`user-select:none`。Playwright 对 dist `http://127.0.0.1:4199/` 验证桌面和手机：handoff 文案包含 505 日窗口、RGTI +417.9% / +671.6%、09:30 压力分钟、SPY cooling；手机 handoff 为 2 列；public risk false，旧锁层 0，旧 `.is-blurred` 0，横向溢出 0，future realtime blur 仅在 `.terminal-table-body.is-realtime-gated`。PNG 导出成功，建议文件名 `kzg-option-house-2026-05-22-zh.png`，大小 `1,482,138` bytes。截图证据：`/tmp/kzg-option-house-v162-dist-desktop.png`、`/tmp/kzg-option-house-v162-dist-mobile.png`、`/tmp/kzg-option-house-v162-realtime-desktop.png`、`/tmp/kzg-option-house-v162-realtime-mobile.png`、`/tmp/kzg-option-house-v162-handoff-mobile.png`。
- iOS blocker: XcodeBuildMCP 能看到 project/scheme/profile，但 `build_run_sim` 仍被本机 simulator destination/runtime 阻塞；shell `xcodebuild -destination 'generic/platform=iOS Simulator'` 报 `iOS 26.5 is not installed`。Swift typecheck 已通过，所以记录为本机 Xcode platform/runtime blocker，不是 Swift 源码 blocker。
- 部署: 本轮不部署生产。生产仍是 v1.60 <https://kzg-option-house.netlify.app/>；唯一生产部署仍是 <https://6a15daeafdbe07993e28b173--kzg-option-house.netlify.app/>。下一次生产部署建议在 v1.63-v1.65 或任何公开风险修复后执行。
- 下一步: v1.63 继续压缩和美化 open historical layer 的入口，重点看 handoff 到跨日趋势、轮动象限、核心标的动量之间的滚动节奏；仍不接真实 key、不做真实升级、不花钱、不提交 raw data。

English:

- Change: Public Web UI moves from `1.61` to `1.62`. Fangbao's current public boundary stays unchanged: generated-minute features remain open; history, trends, rotation, event queues, and PNG export are not blurred, locked, or paywalled; only the future realtime option-flow tape/reserve may keep a reserve-style blur. `public/app.js` adds `realtimeHistoryHandoff()`, bridging the future realtime seat into the free historical layer with an "Open layer handoff" message. It makes clear that the realtime sector is a future seat, while the 505-day historical window below stays open and already exposes volume-premium warming, pressure minutes, and cooling queues.
- Experience: v1.61 shortened the phone realtime reserve; v1.62 answers why users should keep scrolling after that reserve. The handoff bridge is a copy rail plus four compact cells on desktop: 505-day open history, RGTI volume-premium warming, 09:30 pressure minute, and SPY cooling. On phones it becomes a compact two-column grid instead of another long explanation block. `symbolRotationRows()` now has a fallback so development states without symbol history do not render an empty rotation handoff.
- iOS: Native SwiftUI companion syncs from `0.5` to `0.6`. `DashboardView.swift` checkpoint strip shows Web `1.62`, iOS `0.6`, Flow `Future`; new `FlowRouterCard`, `RouterGateTile`, and `TapeChip` mirror the Web Flow Router logic in the native phone surface. iOS still contains no real API key, provider name, payment, domain, price, registration, or account route.
- Verification: `node --check public/app.js` passed; `xcrun --sdk iphonesimulator swiftc -typecheck ios/KZGOptionHouse/KZGOptionHouse/*.swift -target arm64-apple-ios17.0-simulator` passed; `git diff --check -- public/app.js public/styles.css ios/KZGOptionHouse/KZGOptionHouse/DashboardView.swift` passed; public-risk scan across `public/app.js public/index.html public/styles.css ios/KZGOptionHouse/KZGOptionHouse/*.swift` had no matches. `python3 scripts/build_payload.py` produced a `505`-day payload, latest `2026-05-22`, analytics days `505`, analytics symbols `98`, pack asset `kzg-frame-6b392e1504a6.js`; `python3 scripts/per_day_to_dist.py` returned `copied=505`.
- QA: Browser opened local public `http://127.0.0.1:4198/` and confirmed source `23_DATA_期权分钟_Minute · 1.62 · 505/505 complete`, Flow Router, handoff, handoff 505-day copy, 4 gates, 16 tape cells, 0 old lock layers, 0 old `.is-blurred` nodes, 0 horizontal overflow, 0 console issues, and `user-select:none`. Playwright against dist `http://127.0.0.1:4199/` verified desktop and phone: handoff text includes the 505-day window, RGTI +417.9% / +671.6%, 09:30 pressure minute, and SPY cooling; phone handoff is 2 columns; public risk false, old locks 0, old `.is-blurred` 0, horizontal overflow 0, and future realtime blur only on `.terminal-table-body.is-realtime-gated`. PNG export succeeded with suggested filename `kzg-option-house-2026-05-22-zh.png`, size `1,482,138` bytes. Screenshot evidence: `/tmp/kzg-option-house-v162-dist-desktop.png`, `/tmp/kzg-option-house-v162-dist-mobile.png`, `/tmp/kzg-option-house-v162-realtime-desktop.png`, `/tmp/kzg-option-house-v162-realtime-mobile.png`, and `/tmp/kzg-option-house-v162-handoff-mobile.png`.
- iOS blocker: XcodeBuildMCP sees the project/scheme/profile, but `build_run_sim` is still blocked by local simulator destination/runtime selection; shell `xcodebuild -destination 'generic/platform=iOS Simulator'` reports `iOS 26.5 is not installed`. Swift typecheck passes, so this is a local Xcode platform/runtime blocker, not a Swift source blocker.
- Deploy: Not deployed this round. Production remains v1.60 at <https://kzg-option-house.netlify.app/>; latest unique production deploy remains <https://6a15daeafdbe07993e28b173--kzg-option-house.netlify.app/>. Next production deploy should be around v1.63-v1.65 or after any public-risk fix.
- Next: v1.63 should tighten and beautify the open historical-layer entry, especially the scroll rhythm from handoff into cross-day trends, rotation quadrant, and symbol momentum. Still do not connect real keys, perform real upgrades, spend money, or commit raw data.
