...
|---|---|---|---|
| GitHub | Active | Source and docs are pushed on branch `feat/kzg-option-house-daily-auto`. Latest code backup before this ledger is `806c991 tighten option house mobile preview v133`. | Keep committing source/docs only. Do not commit raw generated data under `public/data/`, `public/reports/`, `.private/`, `dist/`, or raw `.csv.gz`. |
| Netlify | Active | Production site is `https://kzg-option-house.netlify.app/`. Latest production deploy is v1.32 at `https://6a155f54945106b4d1611609--kzg-option-house.netlify.app/`. v1.33 is GitHub-only. | Deploy every 3-5 solid versions or immediately after public-risk fixes. Production raw data endpoint `/data/index.json` should remain 404 unless Fangbao explicitly changes policy. |
...
| Browser / Playwright / Chrome | Active for QA | Used local browser automation and headless Chrome/Playwright to verify v1.32 and v1.33 viewports, no horizontal overflow, no public risk terms, and PNG export. | Continue using for visual proof before deploy. Screenshots are QA evidence, not committed artifacts unless explicitly requested. |
