...
|---|---|---|---|
| GitHub | Active | Source and docs are pushed on branch `feat/kzg-option-house-daily-auto`. v1.43 is the latest local mobile lower-page compression checkpoint and should be visible in `git log -1` after backup. | Keep committing source/docs only. Do not commit raw generated data under `public/data/`, `public/reports/`, `.private/`, `dist/`, or raw `.csv.gz`. |
...
| Browser / Playwright / Chrome | Active for QA | Used in-app Browser plus headless Chrome/CDP to verify v1.43 local desktop/mobile viewports, no horizontal overflow, no public risk terms, and mobile lower-page compression. Latest screenshots are `/tmp/kzg-option-house-v143-desktop.png` and `/tmp/kzg-option-house-v143-mobile.png`. | Continue using for visual proof before deploy. Screenshots are QA evidence, not committed artifacts unless explicitly requested. |
