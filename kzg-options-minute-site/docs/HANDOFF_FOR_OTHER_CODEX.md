...
Web `1.66` is the latest local checkpoint; production remains `1.63`. Main source changes are `public/index.html`, `public/app.js`, and `public/styles.css`: UI version `1.66`, new `#bucketRotationBridge` between bucket risk and bucket bars, new `bucketRotationBridge()` in `renderBuckets()`, and responsive/dark styling for the bridge. It connects the intraday pressure bucket, CP extremes, and rotation breadth into one open-history handoff, then uses the existing v1.65 focus behavior to jump into `symbolRotation`. It remains public-open history guidance, not an unlock, subscription, realtime entitlement, provider entry, or payment route. Build produced `505` days, latest `2026-05-22`, pack asset `kzg-frame-ee3731126f01.js`. Playwright on dist `http://127.0.0.1:4207/` passed on desktop and phone: `1.66 · 505/505 complete`, bridge 1, steps 3, jump 1, rotation focus 1 after click, horizontal overflow 0, old `.is-blurred`/`.is-locked` 0, console issue 0, PNG export `1,482,138` bytes. No production deploy happened because v1.64-v1.66 are local open-history rhythm improvements and no public-risk fix was needed. Next v1.67 is the default iOS sync point unless explicitly deferred.

Web `1.66` 是最新本地检查点；生产仍是 `1.63`。主要源码改动是 `public/index.html`、`public/app.js` 和 `public/styles.css`：UI 版本 `1.66`、在 bucket risk 和 bucket bars 之间新增 `#bucketRotationBridge`、在 `renderBuckets()` 里新增 `bucketRotationBridge()`，并给桥补响应式和深色样式。它把日内压力桶、CP 两端和轮动扩散接成一条开放历史接力，再复用 v1.65 的聚焦行为跳到 `symbolRotation`。它仍是公开历史层的阅读导流，不是解锁、订阅、实时权限、供应商入口或支付路线。构建生成 `505` 天，latest `2026-05-22`，pack asset `kzg-frame-ee3731126f01.js`。Playwright dist `http://127.0.0.1:4207/` 桌面和手机通过：`1.66 · 505/505 complete`、bridge 1、steps 3、jump 1、点击后 rotation focus 1、横向溢出 0、旧 `.is-blurred`/`.is-locked` 0、console issue 0、PNG 导出 `1,482,138` bytes。本轮未部署生产，因为 v1.64-v1.66 是本地开放历史阅读节奏增强，且没有公开风险修复需求。下一步 v1.67 是默认 iOS 同步点，除非明确记录延期。

Latest local v1.64 note:
